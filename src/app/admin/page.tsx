...
export default function AdminDashboard() {
  ...
  return (
    <div className="admin-page" style={{ maxWidth: "1100px", margin: "0 auto" }}>
      ...
      <header className="rise admin-header" style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: "16px", marginBottom: "28px" }}>
        <div>
          <span className="section-label" style={{ marginBottom: "12px" }}>
            <Waves size={11} /> Painel · Ivan Silva Surf School
          </span>
      ...
      <div className="admin-grid" style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: "14px", marginBottom: "18px" }}>
        {/* Ocupação semanal */}
      ...
      <div className="card rise-4" style={{ padding: "22px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px" }}>
          ...
          <a href="/admin/financeiro" style={{ fontSize: "12px", color: "var(--teal-light)", fontWeight: 700, display: "inline-flex", alignItems: "center", gap: "2px", textDecoration: "none" }}>
...
function UpcomingRow({ s }: { s: Session }) {
  ...
  const dateLabel = new Date(s.date + "T00:00:00").toLocaleDateString("pt-BR", { day: "2-digit", month: "short" });
  return (
    <div className="card-interactive" style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: "12px" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: "13px", color: "var(--text-1)", fontWeight: 600, display: "flex", alignItems: "center", gap: "5px" }}>
          <MapPin size={11} style={{ color: "var(--teal-light)", flexShrink: 0 }} /> {loc.emoji} {loc.name}
        </p>
...
function PaymentRow({ p, first }: { p: Payment; first: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "14px", padding: "13px 4px", borderTop: first ? "none" : "1px solid var(--border)" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: "13.5px", color: "var(--text-1)", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.customerName}</p>
        <p style={{ fontSize: "11.5px", color: "var(--text-2)", marginTop: "1px" }}>{p.description}</p>
      </div>
